...
case $ROLE in
    a)
        ...
        sync-folders torrent keygen "test-05-bidirectional" > /tmp/keys.txt
...
YAML
        ...
        echo "file from peer-a" > /data/source/from-a.txt
...
YAML
        ...
        mkdir -p /data/source-peer-b
        sync-folders addfolder source-peer-b /data/source-peer-b

        echo "file from peer-b" > /data/source-peer-b/from-b.txt
        # peer-b also pushes its own files
        cat > /tmp/peer-b-push.yaml <<YAML
folder: "source-peer-b"
transport:
  type: torrent
  config:
    client: "qbittorrent"
...
sync:
  direction: "push"
YAML
        sync-folders addconfig /tmp/peer-b-push.yaml
        sync-folders sync peer-b-push

        log_info "peer-b: waiting for DHT manifest"
        wait_for_dht_key "$DHT_PUB" "sync-folders:test-05-bidirectional" 120
